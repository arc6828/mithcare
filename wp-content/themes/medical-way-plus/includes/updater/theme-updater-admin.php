					...
					<tbody>
						...
						<tr valign="top">
							...
							<td>
								...
								if ( 'valid' == $status ) { ?>
									<input type="submit" class="button-secondary" name="<?php echo $this->theme_slug; ?>_license_deactivate" value="<?php esc_attr_e( $strings['deactivate-license'],'medical-way-plus' ); ?>"/>
								<?php } else { ?>
									<input type="submit" class="button-secondary" name="<?php echo $this->theme_slug; ?>_license_activate" value="<?php esc_attr_e( $strings['activate-license'],'medical-way-plus' ); ?>"/>
								<?php }
								?>
							</td>
						</tr>
	 ...
	 */
	 function get_api_response( $api_params ) {
		...
		if ( is_wp_error( $response ) ) {
			wp_die( $response->get_error_message(), __( 'Error','medical-way-plus' ) . $response->get_error_code() );
		}
	...
	function activate_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = __( 'An error occurred, please try again.','medical-way-plus' );
			}
			...
			if ( false === $license_data->success ) {
				...
				switch( $license_data->error ) {
					...
					case 'expired' :
						...
						$message = sprintf(
							__( 'Your license key expired on %s.','medical-way-plus' ),
					...
					case 'revoked' :
						...
						$message = __( 'Your license key has been disabled.','medical-way-plus' );
						break;

					case 'missing' :

						$message = __( 'Invalid license.','medical-way-plus' );
						break;

					case 'invalid' :
					case 'site_inactive' :

						$message = __( 'Your license is not active for this URL.','medical-way-plus' );
						break;

					case 'item_name_mismatch' :

						$message = sprintf( __( 'This appears to be an invalid license key for %s.','medical-way-plus' ), $args['name'] );
						break;

					case 'no_activations_left':

						$message = __( 'Your license key has reached its activation limit.','medical-way-plus' );
						break;
