...
function medical_way_entry_footer() {
	...
	if ( 'post' === get_post_type() && ( 1 != $post_tag ) ) {
		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'medical-way-plus' ) );
		if ( $tags_list ) {
	...
	if ( (! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) && ( 1 != $post_comment ) ) {
		...
		comments_popup_link( sprintf( wp_kses( __( 'No Comment<span class="screen-reader-text"> on %s</span>', 'medical-way-plus' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
		echo '</span>';
	}
	...
	edit_post_link(
		sprintf(
			...
			esc_html__( 'Edit %s', 'medical-way-plus' ),
...
if ( ! function_exists( 'medical_way_primary_navigation_fallback' ) ) :
	...
	function medical_way_primary_navigation_fallback() {
		echo '<ul>';
		echo '<li><a href="' . esc_url( home_url( '/' ) ) . '">' . esc_html__( 'Home', 'medical-way-plus' ) . '</a></li>';
		$args = array(
			'number'       => 4,
			'hierarchical' => false,
			);
